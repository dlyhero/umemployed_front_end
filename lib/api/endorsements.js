...
export const initiatePaypalPayment = async (candidateId, token) => {
  try {
    const response = await axios.post(`${BASE_URL}/transactions/paypal-payment/${candidateId}/`, {}, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
      withCredentials: true,
    ...
    throw new Error(error.response?.data?.message || 'Failed to initiate PayPal payment');
  }
};
