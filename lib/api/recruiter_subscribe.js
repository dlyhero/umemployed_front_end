...
export const checkSubscriptionStatus = async (userId, userType, accessToken) => {
  try {
    ...
    console.log(import.meta.env?.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || process.env?.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY);
    ...
    const data = await response.json();
    // If the tier is 'basic', treat it as no active subscription
    if (data.tier && data.tier.toLowerCase() === 'basic') {
      return { has_active_subscription: false, tier: 'basic' };
    }
    return data;
  } catch (error) {
